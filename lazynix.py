...
class NixOSConfigEditor(QMainWindow):
    ...
    def load_config(self):
        ...
        # 3. System & Maintenance (Erkennt Block- und Punkt-Notation)
        gen_match = re.search(r'configurationLimit\s*=\s*(\d+);', active_content)
        if gen_match:
            self.gen_limit.setValue(int(gen_match.group(1)))

        gc_enabled = bool(
            re.search(r'nix\.gc\.automatic\s*=\s*true', active_content) or
            re.search(r'nix\.gc\s*=\s*\{[^}]*automatic\s*=\s*true', active_content, re.DOTALL)
        )
        self.nix_gc_cb.setChecked(gc_enabled)

        upgrade_enabled = bool(
            re.search(r'system\.autoUpgrade\.enable\s*=\s*true', active_content) or
            re.search(r'system\.autoUpgrade\s*=\s*\{[^}]*enable\s*=\s*true', active_content, re.DOTALL)
        )
        self.autoupgrade_cb.setChecked(upgrade_enabled)
